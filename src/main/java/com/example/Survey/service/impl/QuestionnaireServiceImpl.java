...
//		問卷狀態: 開始時間(0=未開放, 1=開放中, 2=已結束)
		...
		return new QuestionnaireResponse(questionnaireDao.save(survey.get()), "成功");
	}
...
//				開始時間等於今天，狀態改1開始
		...
		return new QuestionnaireResponse(questionnaireDao.saveAll(allSurvey), "成功");
	}
...
//		防呆: 判斷問卷存在
		...
		if(deleteList.size() != deleteSurveyList.size()) {
			return new QuestionnaireResponse("問卷不存在");
		}
//		防呆: 問卷狀態0=未開放 才可刪除問卷
		...
		return new QuestionnaireResponse("成功");
	}
