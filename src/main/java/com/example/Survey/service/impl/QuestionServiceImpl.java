...
//			防呆: 選項不可為空
			if (!StringUtils.hasText(questionList.get(i).getqOption())) {
				return new QuestionResponse("請輸入選項");
			}

		}

		return new QuestionResponse(questionDao.saveAll(questionList), "新增問卷成功");
	}
...
//		防呆: 無此問卷或此問卷無此題目
		if (updateQuestionList.size() != questionList.size()) {
			return new QuestionResponse("無此問卷或題目");
		}
...
//				若原問題流水號與欲修改流水號不符則跳出
				...
				if(!StringUtils.hasText(reqQuestion.getQuestion())) {
					return new QuestionResponse("請輸入問題");
				}
				// 通過則存入新問題
		...
		return new QuestionResponse(questionDao.saveAll(updateQuestionList), "修改成功");
	}
